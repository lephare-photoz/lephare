...
class PlotUtils:
    ...
    catalogs generated by LePHARE, including comparisons between photometric
    ...
    - The table must contain all expected columns from a LePHARE ``outputphotoz.fits`` file.
    ...
    def zml_zs(self):
        """
        Plot photometric redshift (median of the PDF) versus spectroscopic redshift.

        This diagnostic compares the median photometric redshift (`Z_MED`)
        derived by LePHARE with the spectroscopic redshift (`ZSPEC`) for all
    ...
    def zp_zs(self):
        """
        Plot photometric redshift (minimum chi-squared value) versus spectroscopic redshift.

        This diagnostic compares the best-fit photometric redshift (`Z_BEST`)
        obtained from the minimum χ² solution in LePHARE with the spectroscopic
        redshift (`ZSPEC`). It is useful for assessing the quality and bias of
    ...
    def zml_zp(self):
        """
        Plot photometric redshift (median of the PDF) versus photometric redshift (minimum χ²).
    ...
    def distz(self, nstep=20):
        ...
        This diagnostic shows histograms of the spectroscopic redshift (`ZSPEC`),
        the photometric redshift from the minimum χ² solution (`Z_BEST`), and the
    ...
    def absmag_z(self):
        """
        Plot absolute magnitude versus redshift for the sample.

        This diagnostic shows the relation between absolute magnitudes
        (`MAG_ABS()`) in different bands and redshift, providing insight
        into luminosity evolution, selection effects, and completeness
        limits of the survey.

        Notes
        -----
        - Uses the absolute magnitudes in the filters defined by `pos_filt`.
    ...
    def rf_color(self):
        """
        Plot rest-frame colors versus absolute magnitude.

        This diagnostic shows the relation between rest-frame colors (e.g., U–V, B–V)
    ...
    def william(self):
        """
        Create a ''William'' style rest-frame color-color plot.
    ...
    def check_error(self):
        ...
        This diagnostic evaluates whether the estimated photometric redshift
        uncertainties are consistent with the observed scatter relative to
        spectroscopic redshifts. Typically, it checks that approximately 68% of
        spectroscopic redshifts lie within the 68% confidence interval of the
        photometric redshift PDF, as a function of observed magnitude.
