...
class PDF
{
 ...
 public:
  ...
  PDF(const double min, const double max, const size_t size); ///< initialize the pdf with minimal value, maximal value, and total number of points.
  ~PDF(){    vPDF.clear();    chi2.clear();    xaxis.clear(); }
  ...
  pair<double, double>  uncMin(double dchi); ///< compute the bounds of the confidence interval defined by \f$\chi^2_{min} + dchi\f$
  double probaBay(double xvalue); ///< return the inverse cumulative function at xvalue
  size_t index(const double inVal) const; ///< return the vector index where the pdf vector is closest to inVal
  ...
  double levelCumu2x(float xval); // find the xaxis value corresponding to a level in the cumulative function
  pair<double, double> credible_interval(float xval,double redLoc); // Look for the confidence interval around redLoc 
  ...
  inline double linear_interp(double x, double x1, double y1, double x2, double y2){if(x2>x1) return y1 + (x-x1)*(y2-y1)/(x2-x1); else return y1;}
  //for marginalized pdf use
  double int_parabL() {for(size_t k=0; k<vPDF.size(); k++){chi2[k]=-vPDF[k];} return int_parab();}
