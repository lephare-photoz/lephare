/*!
  ...
  All the functions to measure the age of the Universe, the luminosity distance, etc are here
*/
...
public:
  ...
  double time(double z) const;    //!< compute the cosmological time from infinity to z
  inline bool operator==(const cosmo& rhs){return h0==rhs.h0 && om0==rhs.om0 && l0==rhs.l0;}
  inline bool operator!=(const cosmo& rhs){return h0!=rhs.h0 || om0!=rhs.om0 || l0!=rhs.l0;}
  inline friend ostream& operator << (ostream& output, const cosmo& c)
  {output << c.h0 << "," << c.om0 << "," << c.l0; return output;}
};
