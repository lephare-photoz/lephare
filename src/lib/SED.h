...
public:
  ...
  double mass,age,sfr,ssfr,ltir; // need to put it out of GalSED since used in the PDF without knowing that it's a gal.
  ...
  virtual ~SED();
  ///\brief Read sedFile assumed to be ASCII and build the #lamb_flux vector of oneElLambda elements.
  ///
...
public:
  ...
  GalSED(const string nameC, double tauC, double ageC,
  ...
  void clean(){SED::clean(); flEm.clear();}
