/*
  ...
  10/11/14
  Implementation of function for the keyword class (split in a vector of double, etc)
...
vector<double> keyword::split_double(string default_val, int nbItems) {
  ...
  transform(skey.begin(), skey.end(), back_inserter(dkey), [](const string& val) {
    try{
      return stod(val);
    }
    catch(exception const & e)
      {
	throw invalid_argument("Bad input parameter in a keyword. Waiting double rather than: " + val);
      }
  }
    );
...
vector<long> keyword::split_long(string default_val, int nbItems) {
  ...
  transform(skey.begin(), skey.end(), back_inserter(ikey),[](const string& val) {
    try{
      return stol(val);
    }
    catch(exception const & e)
      {
	throw invalid_argument("Bad input parameter in keyword " + nm + ". Waiting long rather than: " + val);
      }
  }
    );
...
vector<int> keyword::split_int(string default_val, int nbItems) {
  ...
  transform(skey.begin(), skey.end(), back_inserter(ikey),[](const string& val) {
    try{
      ...
      {
	throw invalid_argument("Bad input parameter in a keyword. Waiting integer rather than: " + val);
      }
  }
    );
...
vector<string> keyword::split_string(string default_val, int nbItems) {
  ...
  if(value.empty()) { value = default_val;}
  ...
  if(nbItems>1 && nbElems==1){
    ...
    for (int k=1 ; k<nbItems ; k++){elems.push_back(elems[0]);};
  }

  //Check if the number of expected values correspond to the number of read values
  ...
  if(nbItems>0 && nbElems!=nbItems){
    ...
    elems.clear();
    for (int k=0 ; k<nbItems ; k++){elems.push_back(default_val);};
  }
...
// For instance, if a keyword is defined as $HOME/bid.dat, this function replace HOME by its actual value
void keyword::expand_path() {
  ...
  // Check if the LEPHAREDIR environment variable is present at the beginning of the string 
  string str=string("LEPHAREDIR");
  // Search the string
  size_t found = value.find(str); 
  // if found : replace by using the environment variable
  if(found != string::npos) value.replace(value.find(str)-1,str.length()+1,getenv(LEPHAREDIR.c_str()));

  // Check if the HOME environment variable is present at the beginning of the string 
  str=string("HOME");
  // Search the string
  found = value.find(str); 
  // if found : replace by using the environment variable
  if(found != string::npos) value.replace(value.find(str)-1,str.length()+1,getenv(HOME.c_str()));
  ...
  for (int k=1; k<argc ; k=k+2)
	...
	if(lit.at(0)=='-'){
	  if(k+1<argc) {
	    ...
	    throw runtime_error("Problem in format of the command line. Use one keyword followed by one value ");
	  }
	}
      }
    }
  
  cout << "Number of keywords read at the command line (excluding -c config): " << nb_keywords << endl;
...
keymap analyse_keywords(int argc, char *argv[], const string * list_keywords, int nb_ref_key)
{
