...
void flt::read(ifstream& sfiltIn) {
  ...
  if (firstChar != '#'){
    ...
    //Loop over each lambda, according to the number of lines indicated in comment
...
void flt::clean() {
    ...
    // Remove the extreme elements of the filter as long as the transmission is below 1% of the maximum transmission
    // From the end of the filter to the first significant transmission
    ...
    //Add 0 value at the two extreme ends of the filter, 10 Angtrom below and above
...
double flt::peak() {
   ...
   sort (sortVec.begin(),sortVec.end(),[] (oneElLambda i,oneElLambda j) { return (i.val>j.val); }); 
...
double flt::lambdaMean() {
   ...
   for (int k=0; k<nb-1; ++k){
     double trans   =((lamb_trans[k]).val+(lamb_trans[k+1]).val)/2.;  // averaged transmission
     double diffLamb=(lamb_trans[k+1]).lamb-(lamb_trans[k]).lamb;     // step in lambda
     double avLamb  =((lamb_trans[k+1]).lamb+(lamb_trans[k]).lamb)/2.; // average lambda
...
double flt::width() {
  ...
  if (name == "Heavy") { return lmax() - lmin() -2 ; }
   ...
   // Iterate over each element in increasing lambda -> find the lower limit in lambda at T=0.5
   int nb=lamb_trans.size();
   for (int k=0; k<nb; ++k){
     oneElLambda elP0(lamb_trans[k]);
     oneElLambda elP1(lamb_trans[k+1]);
     //If the transmission of the P0 element below the threshold while the P1 element is above
   ...
   // Iterate over each element in decreasing lambda -> find the higher limit in lambda at T=0.5
   for (int k=nb-1; k>0; --k){
     oneElLambda elP0(lamb_trans[k-1]);
     oneElLambda elP1(lamb_trans[k]);
     //If the transmission of the P0 element above the threshold while the P1 element is below
     if(elP0.val>treshold && elP1.val<treshold){
       //interpolation
       slope=(elP1.lamb-elP0.lamb)/(elP1.val-elP0.val);
...
double flt::lambdaEff() {
  ...
  SED vegSED("Vega");  
   ...
   // integrate the vega spectrum over the filter considered in this instance of the object
   // Various information on the integral is stored within magVeg
...
double flt::tgcorr() {
   ...
   tgSED.read(tgFile);
   // integrate the BD17 spectrum over the filter considered in this instance of the object
   // Various information on the integral is stored within magTG
   vector<double> magTG=tgSED.integrateSED(*this);

   //mtg(*) = mvega(*) + TGcor : TGcor=2.5log(Int[F(BD)TdL]/Int[F(Vega)TdL]) +9.50-0.03
   if(magTG[3]>0)tg=2.5*log10(magTG[3]/magVeg[3])+9.5-0.03; else tg=-99;
...
double flt::magsun() {
   ...
   */
   if(magSun[3]>0)msun=-2.5*log10(magSun[3]/magSun[1])+68.6227-48.6+31.572;
...
/*
 ...
 fcorr= Fcalib(leff) * leff^2 *  (int (T / lambda^2) dLambda /  int ( Fcalib T ) dLambda )
*/
double flt::fcorrec() {

   // Generate a SED which is used as a reference (Fcalib) depending on the keyword "CALIB" 
   // Define a lambda range of 1000 steps between lambda min and max of the filter
   SED calibSED("calib");
   calibSED.generateCalib(lmin()-10,lmax()+10,1000,calibtyp);

   // Define the second part of the correction (Fcalib is the SED used as a reference)
   // int (T / lambda^2) dLambda /  int ( Fcalib T ) dLambda 
   vector<double> magCalib=calibSED.integrateSED(*this);
   fcorr = magCalib[5]/magCalib[3];

   //If calib=4 or 5, take nuFnu=cst (case calib 1) to compute lambda_eff -> change the calibration SED
...
double flt::lambdaEff2() {
   ...
   //If calib=4 or 5, take nuFnu=cst (case calib 1) to compute lambda_eff -> change the calibration SED
   if(calibtyp==4 || calibtyp==5)calibSED.generateCalib(lmin()-10,lmax()+10,1000,1);
...
void write_output_filter (string &filtfile, string &filtdoc, vector<flt> vecFlt)
{
  ...
  if(!stfiltdoc){
   ...
   //header for the .doc
   stfiltdoc << "# Name id Lambda_mean Lambda_eff width AB Vega fcorr transtype calib Nb_lines "  << endl ;
   ...
   for (vector<flt>::iterator it=vecFlt.begin(); it<vecFlt.end(); ++it){
     ...
     for (vector<oneElLambda>::iterator it2=oneFlt.begin(); it2<oneFlt.end(); ++it2){
       stfiltfile << it2->lamb << "  " << it2->val << "  " << ((it->id)+1)  << endl;
     }
...
create a vector of filters from the documentation file including all informations as the AB correction, etc.

Input: name of the input file
Output: vector including all filters, and a boolan to indicate if the documentatio file is present
*/
vector<flt> read_doc_filters(const string filtFile)
{
  ...
  if(!(Fexist)){
    throw invalid_argument("Filter documentation does not exist in LEPHAREWORK/filt/" + filtFile);
  }
