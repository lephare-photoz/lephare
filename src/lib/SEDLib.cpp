/*
  ...
  27/11/14
  Implementation of the functions to read the various SED files depending on their format
  The output should be a vector of "SED" (one SED for a simple ascci file and a vector of SED when several templates are included (e.g. BC03)
...
// prototypes 
vector<GalSED> readBC03(string sedFile, int nummod, string type, vector<double> & ageSel);
vector<GalSED> readPEGASE(string sedFile, int nummod, string type, vector<double> & ageSel);
...
vector<GalSED> readBC03(string sedFile, int nummod, string type, vector<double> & ageSel) {
 ...
 ssed >> nages ; 
 for(int k=0;k<nages;k++) {  ssed >> dage ; age.push_back(dage);}
 ...
 double ml,mu,xx,lm,um, baux, cn,cc,totm, totn,avs,tau,tau1,tau2,tau3,tau4;
 char stelib;
 ssed >> ml >> mu >> iseg;
 for(int k=0;k<iseg;k++) {ssed >> xx >> lm >> um >> baux >> cn >> cc;}
 ssed >> totm >> totn >> avs >> jo >> tau >> tau1 >> tau2 >> tau3 >> tau4 >> iop >> stelib ;
 // Skip the 3 lines 
 getline(ssed,id); // add this one but it should not be ... Probably the end of the previous line
 getline(ssed,id);  
 double pos, zmett; 
 string zme; 
 pos=id.find("Z="); // find the position of "Z="
 zme = id.substr(pos + 2, 10); // read in id the string starting in pos+2 during 10 char.
 ...
 ssed  >> inw ;
 for(int k=0;k<inw;k++) {ssed >> dw ; w.push_back(dw); }
 // Loop over the ages
 ...
 for(int i=0;i<nages;i++) {
     ...
     ssed >> ik ; 
     if(ik != inw){throw runtime_error("Number of wavelength different from the number of flux in BC03 ");}
     ...
     ssed >> ix ;
     for(int k=0;k<ix;k++) {ssed >> df ; f.push_back(df);}
     //The core of the SED is created (its name, age, tau and lambda/flux), physical parameters added later
     if(useAge[i]) {outSED.push_back(oneSED);}
 }

 /*
  Final physical information stored at the end of the BC03 file, include the relevant one in each SED
  ix represent the number of ages
 */
 double dbol,dmstr,dsf,devf,dsnr,dpnr,dbh,dsn,dwd,drm;
 vector<double> vmass,vsfr;
 ssed >> ix ; for(int k=0;k<ix;k++) {ssed >> dbol ; }
 ssed >> ix ; for(int k=0;k<ix;k++) {ssed >> dmstr; if(useAge[k])vmass.push_back(dmstr);}  // Mass-to-light ratio
 ssed >> ix ; for(int k=0;k<ix;k++) {ssed >> dsf  ; if(useAge[k])vsfr.push_back(dsf)   ;}      // star formation history
 ssed >> ix ; for(int k=0;k<ix;k++) {ssed >> devf ; }
 ssed >> ix ; for(int k=0;k<ix;k++) {ssed >> dsnr ; }
 ssed >> ix ; for(int k=0;k<ix;k++) {ssed >> dpnr ; }
 ssed >> ix ; for(int k=0;k<ix;k++) {ssed >> dbh  ; }
 ssed >> ix ; for(int k=0;k<ix;k++) {ssed >> dsn  ; }
 ssed >> ix ; for(int k=0;k<ix;k++) {ssed >> dwd  ; }
 ssed >> ix ; for(int k=0;k<ix;k++) {ssed >> drm  ; }
...
vector<bool> closeAge(vector<double> ageSel, vector<double> age) { 
  ...
  if(ageSel.size()==2){
    for (vector<double>::iterator it=age.begin(); it<age.end(); ++it)outAge.push_back(true);
  }else{
   // Initialise the output vector at 0 (age not used)
    for (vector<double>::iterator it=age.begin(); it<age.end(); ++it)outAge.push_back(false);
  }
  ...
  for (vector<double>::iterator iti=ageSel.begin()+2; iti<ageSel.end(); ++iti){
   ...
   for (vector<double>::iterator itj=age.begin(); itj<age.end(); ++itj){
     ...
     if( diff < dage && diff < 2.e9 && *itj>=agemin && (*itj<=agemax || agemax<=0)){
       dage=diff;
       kmin=k;
     }
     k++;
