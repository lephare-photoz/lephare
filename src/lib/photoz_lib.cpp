...
PhotoZ::PhotoZ(keymap& key_analysed)
{
  ...
  // GLB_CONTEXT Global context to be used for all objects - 0 default (all bands)
  ...
  if(zrange[0]<0 || zrange[1]<0){zrange[0]=0; zrange[1]=10000;}
  ebvrange[0]  =((keys["EBV_RANGE"]).split_double("0",2))[0];
  ebvrange[1]  =((keys["EBV_RANGE"]).split_double("10000",2))[1];
  if(ebvrange[0]<0 || ebvrange[1]<0){ebvrange[0]=0; ebvrange[1]=10000;}
  ...
  if(magabsB[0]>magabsF[0]){double inter = magabsF[0]; magabsF[0]=magabsB[0]; magabsB[0]=inter;}
  
  // MAG_ABS_QSO allowed range in absolute magnitude for QSO library - 0 default (all bands)
  magabsB[1]  =((key_analysed["MAG_ABS_QSO"]).split_double("0.",2))[0];
  magabsF[1]  =((key_analysed["MAG_ABS_QSO"]).split_double("0.",2))[1];
  // be sure that magabsB is the minimum value
  if(magabsB[1]>magabsF[1]){double inter = magabsF[1]; magabsF[1]=magabsB[1]; magabsB[1]=inter;}
  ...
  babs=babs-1;
  if(babs<0){ magabsB[0]=99.; magabsF[0]=99.;magabsB[1]=99.; magabsF[1]=99.;}
  ...
  // Z_METHOD compute the absolute magnitude at a given redshift solution ML or BEST - BEST by default
  ...
  // FIR_FREESCALE possible free rscaling in IR, when several bands. Otherwise, model imposed by its LIR
  ...
  // Need to substract one because the convention in the .para file start at 1, but 0 in the code
  ...
  /*  AUTO-ADAPT */
  // APPLY_SYSSHIFT shift to be applied in each filter. Create a fake shifts1 vector 
  ...
  */
  ...
  outputHeader +=   "# CAT_LINES              : " + to_string(rowmin) + ' ' + to_string(rowmax) +'\n';
  ...
  for (int k=0 ; k<numlib ; k++){outputHeader +=  colib[k] + ' ' ;};
  outputHeader += '\n';
  outputHeader +=   "# FIR_LIB                : ";
  for (int k=0 ; k<nlibext ; k++){outputHeader +=  libext[k] + ' ' ;};
  outputHeader += '\n';
  ...
  for (int k=0 ; k<nerr ; k++){outputHeader +=  to_string(min_err[k]) + ' ' ;};
  outputHeader += '\n';
  outputHeader +=   "# ERR_FACTOR             : " ;
  for (int k=0 ; k<nfac ; k++){outputHeader +=  to_string(fac_err[k]) + ' ' ;};
  outputHeader += '\n';
  ...
  outputHeader +=   "# MAG_ABS                : " + to_string(magabsB[0]) + ' ' + to_string(magabsF[0]) +'\n';
  outputHeader +=   "# MAG_ABS_AGN            : " + to_string(magabsB[1]) + ' ' + to_string(magabsF[1]) +'\n';
  outputHeader +=   "# MAG_REF                : " + to_string(babs) +'\n';
  outputHeader +=   "# NZ_PRIOR               : " + to_string(bp[0]) + ' ' + to_string(bp[1]) +'\n';
  outputHeader +=   "# Z_INTERP               : " + bool2string(zintp) +'\n';
  outputHeader +=   "# Z_METHOD               : " + methz+'\n';
  outputHeader +=   "# PROB_INTZ              : ";
  for (int k=0 ; k<npdz ; k++){outputHeader +=  to_string(int_pdz[k]) + ' ' ;};
  outputHeader += '\n';

  outputHeader +=   "# MABS_METHOD            : " + to_string(method)+'\n';
  outputHeader +=   "# MABS_CONTEXT           : ";
  for (int k=0 ; k<nmagabscont ; k++){outputHeader +=  to_string(magabscont[k]) + ' ' ;};
  outputHeader += '\n';
  outputHeader +=   "# MABS_REF               : ";
  for (int k=0 ; k<nbapp ; k++){outputHeader +=  to_string(bapp[k]) + ' ' ;};
  outputHeader += '\n';
  ...
  outputHeader +=   "# ADAPT_LIM              : " + to_string(auto_thresmin) + ' ' + to_string(auto_thresmax) +'\n';
  ...
  outputHeader +=   "# ADAPT_ZBIN             : " + to_string(adzmin) + ' ' + to_string(adzmax) +'\n';
  ...
  */
  ...
  for(int j=nlibext-1;j>=0;j--){
    read_lib(fullLibIR,indFIR,nummodpreFIR,libext[j],filtNameFIR,emMod,babs);
  }
  cout << "Read filt " << endl;
  ...
  string filtNameAdd = (key_analysed["ADDITIONAL_MAG"]).split_string("none",1)[0];
...
keymap read_keymap_from_doc(const string libName) {
  ...
  string list_keywords[] =  {"LIB_TYPE","NUMBER_ROWS","FILTER_FILE","FILTERS","EM_LINES","LIB_NAME","NUMBER_SED","ZGRID_TYPE","Z_STEP","COSMOLOGY","EXTINC_LAW","EB_V","MOD_EXTINC","Z_FORM"};
  //Number of keywords
  ...
  //Find the reference keywords within the command line + config file -> store them into a vector
  ...
  if(access( docOutFile.c_str(), F_OK ) == -1 ){
    throw invalid_argument("The doc file " + docOutFile + " does not exist. Stop.");}
  // need to create an array of char pointer, exactly as argv, in order to use analyse_keywords
...
void PhotoZ::check_consistency(keymap& keys) {
  ...
  if(valc[0] != 'S' && valc[0] != 's'){
	...
	throw runtime_error("Cosmology and redshift grids parameters are not consistent in the input libraries!");
      }
    }
  }
}
...
*/
void PhotoZ::read_lib(vector<SED*> & libFull, int & ind, int nummodpre[3], const string libName, string & filtname, vector<int> emMod, int & babs)
{ 
  ...
  //get the keywords from the saved doc files in order to ensure library consistency
  keymap key_analysed = read_keymap_from_doc(libName);
  //build cosmology and gridz, and verify that they are identical across GAL and QSO libraries
  ...
  // EXTINC_LAW"
  vector<string> extlaw = (key_analysed["EXTINC_LAW"]).split_string("calzetti.dat",-1);
  int nextlaw = int(extlaw.size());
  //EB_V
  vector<double> ebv = (key_analysed["EB_V"]).split_double("0",-1);
  // MOD_EXTINC
  vector<int> modext = (key_analysed["MOD_EXTINC"]).split_int("0,0",nextlaw*2);
  ...
  if(!slibIn){throw invalid_argument("Library doesn't exist ");}
  ...
  while(slibIn.tellg() < length ){
    ...
    // Keep in the library only redshift or EBV in of the prior range (except 0 in all cases).
    if((oneSED->red>=zrange[0] && oneSED->red<=zrange[1]) || oneSED->red<1.e-20){
     if((oneSED->ebv>=ebvrange[0] && oneSED->ebv<=ebvrange[1]) || oneSED->ebv<1.e-20){
        // Case in which we add the emission lines (only for galaxies), if option turned on
        if((valc[0] == 'G' || valc[0] == 'g') && (emlines[0]=='P' ||emlines[0]=='E')){ 
          // Check if we are well in the considered model range to add lines
          if(emMod[0]>=0 && emMod[1]>=0 && (oneSED->nummod)>=emMod[0] && (oneSED->nummod)<=emMod[1]){
      	    oneSED->sumEmLines();
          }else{
            // Loop over the lines: put them at 0 since out of the considered model range
            if(oneSED->red==0){for(int k=0;k<65;k++){oneSED->fac_line[k].val=0.;}}
	  } 
        }
        // Index la SED
        (oneSED->index)=ind;
        ...
        if(oneSED->red==0){
          ...
          mag_z0.clear();
          for (int k=0;k< int((oneSED->mag).size()) ;k++)mag_z0.push_back(oneSED->mag[k]);
        }
        ...
        // Recompute the kcorr because of the emission lines not included in mag_gal
  ...
  // keep the last model number to start the numbering with it at the next library
  nummodpre[libFull.back()->nlib]=(libFull.back())->nummod;
  slibIn.close();
  cout << " Done with the library reading with " << libFull.size()<<" SED read. " << endl;
...
vector<string> readOutKeywords(const string outpara){
  ...
  if (outpara == nonestring){
    cout<<"WARNING: PARA_OUT not set or set to NONE, output will contain all the parameters resulting from the fit."<<endl;
    ...
    if(!stoutpara){
      throw invalid_argument("Incorrect setting for PARA_OUT: can't open output parameter file "+ outpara);
    }
...
string PhotoZ::prep_header(vector<string> outkeywords){
  ...
  for(const auto& outkey : outkeywords){
    ...
    for(const string k : {"IDENT","CONTEXT","NBAND_USED","NBAND_ULIM","ZSPEC","STRING_INPUT"})
      {	if(outkey == k) {form += " " + k + " " + to_string(numCol); numCol++;} }
    
    for(const string k : {"Z", "ZQ"}){
      for(const string k1 : {"BEST", "MED", "MODE"}){
	for(const string k2 : {"", "68_LOW", "68_HIGH", "90_LOW", "90_HIGH", "99_LOW", "99_HIGH"})
	  { if(outkey == k+"_"+k1+k2) {form += " " + outkey + " " + to_string(numCol); numCol++;} }
      }
    }
    
    for(const string k : {"MOD", "EXTLAW", "EBV", "MASS","LDUST","LUM_TIR", "LUM_NUV", "LUM_R", "LUM_K", "PDZ", "CHI", "SCALE", "AGE","SFR", "SSFR"})
      { if(outkey == k+"_BEST")         {form += " " + outkey + " " + to_string(numCol); numCol++;} }
    
    for(const string k : {"MAG_OBS", "ERR_MAG_OBS", "MAG_MOD","MABS_FILT","K_COR","MAG_ABS","EMAG_ABS"})
      { if(outkey == k+"()"){form +=  " " + outkey + to_string(numCol); numCol+=imagm; form += "-"+to_string(numCol-1);} }

    for(const string k : {"MAG_PRED","ABSMAG_PRED",})
      { if(outkey == k+"()" && imagmAdd!=0) {form += " " + outkey + to_string(numCol) ; numCol+=imagmAdd; form += "-"+to_string(numCol-1);} }
    
    if(outkey == "EM_FLUX()") {form +=" EM_FLUX() " + to_string(numCol); numCol+=65; form += "-"+to_string(numCol-1);}

    for(const string k : {"MOD_QSO","MOD_STAR","CHI_QSO","CHI_STAR","LIMITS_ZMAX","LIMITS_MFAINT"})
      { if(outkey == k) {form += " " + k + " " +  to_string(numCol) ; numCol++;} }
    
    for(const string k : {"MOD","EXTLAW","Z","EBV","PDZ","CHI","SCALE","AGE"}){
      if(outkey == k+"_SEC")         {form += " " + outkey + " " + to_string(numCol); numCol++;}
    }

    for(const string k1 : {"AGE","LDUST","LUM_TIR","MASS","SFR", "SSFR","COL1","COL2","MREF"}){
      for(const string k2 : {"MED","INF","SUP"}){
	if(outkey == k1+"_"+k2)         {form +=" "+outkey+" " + to_string(numCol); numCol++;}
      }
    }
    
    for(const string k1 : {"EM_FLUX", "EM_EW"}){
      for(const string k2 : {"LYA", "OII", "HB", "OIIIA", "OIIIB", "HA", "SIIIA", "SIIIB"}){
	if(outkey == k1+"_"+k2)     {form +=" "+outkey+" " + to_string(numCol); numCol++;}
      }
    }
  ...
  for(const auto& outkey : outkeywords){
	...
	for(const string k2 : {"", "68_LOW", "68_HIGH", "90_LOW", "90_HIGH", "99_LOW", "99_HIGH"})
	  { if(outkey == k+"_"+k1+k2){form +=  " "+outkey+" ";} }
      }
    }
    //legacy
    if(outkey == "Z_QSO"){form += " ZQ_BEST ";}
    
    for(const string k : {"MOD", "EXTLAW", "EBV", "MASS","LDUST","LUM_TIR", "LUM_NUV", "LUM_R", "LUM_K", "PDZ", "CHI", "SCALE", "AGE","SFR", "SSFR"})
      { if(outkey == k+"_BEST"){form +=" "+outkey+" ";} }
    
    for(const string k : {"MAG_OBS", "ERR_MAG_OBS", "MAG_MOD","MABS_FILT","K_COR","MAG_ABS","EMAG_ABS"})
      { if(outkey == k+"()"){for (int l=0; l<imagm; l++)form +=  " " + k + to_string(l) + " ";} }
    
    for(const string k : {"MAG_PRED","ABSMAG_PRED",})
      { if(outkey == k+"()") {for (int l=0; l<imagmAdd; l++)form += " " + k + to_string(l) + " ";} }
    
    if(outkey == "EM_FLUX()"  ) {for (int l=0; l<65; l++)form += " EM_FLUX" + to_string(l) + " ";}
    
    for(const string k : {"MOD_QSO","MOD_STAR","CHI_QSO","CHI_STAR","LIMITS_ZMAX","LIMITS_MFAINT"})
      { if(outkey == k) {form += " " + k + " ";} }
    
    for(const string k : {"MOD","EXTLAW","Z","EBV","PDZ","CHI","SCALE","AGE"})
      { if(outkey == k+"_SEC"){form += " " + outkey + " ";} }
    
    for(const string k1 : {"AGE","LDUST","LUM_TIR","MASS","SFR", "SSFR","COL1","COL2","MREF"}){
      for(const string k2 : {"MED","INF","SUP"})
	{ if(outkey == k1+"_"+k2){form +=" "+outkey+" ";} }
    }
  ...
  while(getline( sin, line)){
    ...
    if(test_first_char(line)) {
      ...
      if(oneObj->zs>adzmin && oneObj->zs<adzmax){
        // Correct the observed magnitudes and fluxes with the coefficients given in APPLY_SHIFTS
...
PhotoZ::run_autoadapt(vector<onesource*> adaptSources)		      
{
  ...
  if(verbose) cout << "\n Number of sources read for auto adapt " << adaptSources.size() << endl;
  ...
  if(adaptSources.size()>0){
   ...
   while(converge==0 && iteration<10){
      ...
      for(auto& oneObj : adaptSources){
        // Correct the observed magnitudes and fluxes with the coefficients found by auto-adapt
        ...
        oneObj->considered_red(zfix, methz);
      	...
      	// Select the valid index of the library in case of ZFIX=YES to save computational time
        valid = oneObj->validLib(fullLib, zfix,oneObj->zs);
        // Fit the source at the spec-z value
      	oneObj->fit(fullLib,flux,valid,funz0,bp);
        // Interpolation of the predicted magnitudes, scaling at zs, checking first that the fit was sucessfull
        if(oneObj->indmin[0]>=0){
          oneObj->zmin[0]=oneObj->zs;
          oneObj->interp_lib(fullLib,imagm,lcdm);
        }
        if(verbose) cout << " Fit source for adapt " <<  oneObj->spec << "  \r " << flush;
      }
      // run auto-adapt
...
*/
void auto_adapt(const vector<onesource*>  adaptSources, vector<double> & a0, vector<double> & a1, int & converge, int & iteration)
{ 
  ...
  // Store the median of the difference in each band between the predicted and the observed magnitudes
  for(int k=0;k<imagm; k++){
    diff.clear();
    // define a vector difference between the observed and predicted mag
     for(auto& oneObj : adaptSources){
      ...
      if(oneObj->mab_ori[k]>0 && oneObj->indmin[0]>0 && oneObj->busnorma[k]==1){
	// Same convention as before. Need to be added to predicted magnitude, or substracted to observed magnitudes
    ...
    if(diff.size()>10){
      ...
      sort (diff.begin(),diff.end(),[] (double a,double b) { return (a>b); });  
  ...
  // Compare the precedent value of a0 with the new ones to check for convergence
  double somme=0.;
  for(int k=0;k< imagm; k++) somme+=abs(a0[k] - a0pre[k]) ;
  if(somme/double(imagm)<0.02)converge=1; // average diff over the filter lower than 0.02
  // If one band varies by more than 0.03 mag, go on with iteration
  for(int k=0;k< imagm; k++)if(abs(a0[k] - a0pre[k])>0.03)converge=0 ;
  ...
  cout << " Done with iteration " << iteration << " and converge " << converge << endl;
...
*/
vector<vector<int>> bestFilter(int nbFlt, vector<double> gridz, vector<SED*> fulllib, int method,vector<long> magabscont,vector<int> bapp,vector<int> bappOp,vector<double> zbmin, vector<double> zbmax ){
  ...
  for(size_t i=0; i<gridz.size();i++){
    ...
    switch (method) {
      ...
      // Use the same filter for the apparent magnitude as for the  absolute one
    case 0:
      for (int k=0; k<nbFlt; ++k){bestFlt_z.push_back(k);}
      break;

      // method 2 : Fix bapp as observed-frame
    case 2:
      if(bapp.size()==(size_t)nbFlt){
        for (int k=0; k<nbFlt; ++k){bestFlt_z.push_back(bapp[k]);}
      }else{
        for (int k=0; k<nbFlt; ++k){bestFlt_z.push_back(bapp[0]);}
      }
      break;
    ...
    case 4:
      j=0;
      // change the redshift bin as long as we are not in the right redshift bin
      while(!(gridz[i]>=zbmin[j] && gridz[i]<zbmax[j]) && (size_t)j<zbmax.size()-1)j++;
      for (int k=0; k<nbFlt; ++k){bestFlt_z.push_back(bappOp[j]);}
      break;

      // method 3, 1 and default
    default:
      for (int k=0; k<nbFlt; ++k){bestFlt_z.push_back(-1);}
    }
...
/*
  Derive the k-color term (rest-frame color - k-correction) maximum, in order to conserve the error bars for the absolute magnitudes
*/
vector<vector<double>> maxkcolor(vector<double> gridz, vector<SED*> fulllib, vector<vector<int>> bestFlt ){
...
#pragma omp for
	...
	for (size_t k=0; k<bestFlt[0].size(); ++k){ 
	  ...
	  if(bestFlt[indz][k]>=0){
	    kcolor= ((fulllib[(*it)->index_z0])->mag[k]) - ((fulllib[(*it)->index_z0])->mag[bestFlt[indz][k]]) - (*it)->kcorr[bestFlt[indz][k]];
	    //kcolor= (*it)->mag[k] - (*it)->mag[bestFlt[indz][k]] - (*it)->kcorr[k]; 
	    //Store it if the value is maximum or minimum
	    if(extremeDiffMin[indz][k]>kcolor)extremeDiffMin[indz][k]=kcolor;
	    if(extremeDiffMax[indz][k]<kcolor)extremeDiffMax[indz][k]=kcolor;
	  }
	}
      }
    }
...
/*
  Define the filters to pick (depending on the redshift) to minimize the k-correction + rest-frame color term in the absolute magnitude computation
*/
void minimizekcolor(vector<double> gridz, vector<SED*> fulllib, vector<vector<int>> & bestFlt, vector<long> magabscont){
  ...
  // bestFlt has been initialize at -1 before the call, with a dimention redshift grid/nb filt
...
#pragma omp for
	...
	if((*it)->nlib==0){ 
	  ...
	  for (int l=0; l<nbPossi; ++l){ 
	    // derive the k-color term at this redshift: (m(ref)-m(obs))(z=0)-kcorr(obs)
	    kcolor= ((fulllib[(*it)->index_z0])->mag[r]) - ((fulllib[(*it)->index_z0])->mag[possiFlt[l]]) - (*it)->kcorr[possiFlt[l]];
	  
	    //Store it if the value is maximum or minimum
	    if(extremeDiffMin[indz][possiFlt[l]]>kcolor)extremeDiffMin[indz][possiFlt[l]]=kcolor;
	    if(extremeDiffMax[indz][possiFlt[l]]<kcolor)extremeDiffMax[indz][possiFlt[l]]=kcolor;
	  }
	}
      }
#ifdef _OPENMP
	...
	// keep the filter which is able to minimize the k-color, save it into bestFlt
	if((extremeDiffMax[i][possiFlt[k]]-extremeDiffMin[i][possiFlt[k]])<mini){
	  bestFlt[i][r]=possiFlt[k];
	  mini=(extremeDiffMax[i][possiFlt[k]]-extremeDiffMin[i][possiFlt[k]]);
	}
      }
    }
...
vector<onesource*> PhotoZ::read_photoz_sources(){
  ...
  if ( externalzfile.substr(0, 4)!="NONE"){
    ...
    if(!szex){cout << "External spec-z option, but no file " << externalzfile << endl; exit(0);}
    string linezex;
    // Ignore the comments
    int nbcomments =0;
    while(!(test_first_char(linezex))){ getline( szex, linezex ); nbcomments++ ;}
    // back to the beginning of the file
    szex.seekg(0, ios::beg);;
    // Go directly to the right lines, skip commented lines
    for(int k=1;k<nbcomments;k++){getline( szex, linezex ); cout << "skip comments " << '\n';} //go to the right starting row of the file
    // Go directly to the right lines, skipping lines if CAT_LINES
    for(unsigned int k=1;k<rowmin;k++){getline( szex, linezex ); cout << "done skip " << k << " " << rowmin << '\n';} //go to the right starting row of the file
  }
  ...
  // open the input file
  ifstream sin(cat.c_str());
  if(!sin){cout << "No input file " << cat << endl; exit(0);}
  // Back to the beginning of the file
  sin.clear();
  sin.seekg(0,sin.beg);
  while(getline( sin, line)){
    ...
    if(test_first_char(line)) {
      nobj++;
      if((nobj<rowmin && rowmin>0) || (nobj>rowmax && rowmax>0)){ continue; }  //CAT_LINES option
	...
	sszex >> idzex;
	if(idzex!=oneObj->spec) cout << endl << "ERROR: mismatch in the external file " << idzex << " " << oneObj->spec << endl;
	sszex >> oneObj->zs;
      }
...
/*
  Additional layer to prepare the data for the run (context, flux and asscoiated uncertainties)
*/
void PhotoZ::prep_data(onesource* oneObj){
  // Convert the magnitude in fluxes if needed
  if(typm[0] == 'M')
    oneObj->convertFlux(catmag, allFilters);
...
*/
void PhotoZ::run_photoz(vector<onesource*> sources,
			const vector<double>& a0, const vector<double>& a1)
{
  // Open the output file 
  // AUTO_ADAPT adaptation of the zero-points
  bool autoadapt = keys["AUTO_ADAPT"].split_bool("NO",1)[0];
  // RM_DISCREPANT_BD
  // Threshold in chi2 to consider. Remove <3 bands, stop when below this chi2 threshold 
  double thresholdChi2  =((keys["RM_DISCREPANT_BD"]).split_double("1.e9",2))[0];
  ...
  // Need to substract one because the convention in the .para file start at 1, but 0 in the code
  if(fltColRF.size()>1){for(int k=0;k<4;k++)fltColRF[k]--;}  
  // M_REF compute the absolute magnitudes and associated errorbars
  int fltREF = ((keys["M_REF"]).split_int("0",-1))[0];
  // Need to substract one because the convention in the .para file start at 1, but 0 in the code
  if(fltREF>=0){fltREF--;}  
  // LIMITS_ZBIN Compute the z_max and M_faint in several bins of redshift. Give the z bin. 
  vector<double> limits_zbin = (keys["LIMITS_ZBIN"]).split_double("0.0,90.",-1);
  int nzbin = int(limits_zbin.size())-1;
  // LIMITS_MAPP_REF Compute the z_max and M_faint in several bins of redshift. Give the reference band.
  int limits_ref = ((keys["LIMITS_MAPP_REF"]).split_int("1",1))[0];
  // LIMITS_MAPP_SEL Compute the z_max and M_faint in several bins of redshift. Give the selection band in each bin. 
  vector<int> limits_sel = (keys["LIMITS_MAPP_SEL"]).split_int("1",nzbin);
  // LIMITS_MAPP_CUT Compute the z_max and M_faint in several bins of redshift. Give the cut in magnitude in each bin. 
  vector<double> limits_cut = (keys["LIMITS_MAPP_CUT"]).split_double("90.",nzbin);
  ...
  vector<vector<int>> goodFlt = bestFilter(imagm,gridz,fullLib,method,magabscont,bapp,bappOp,zbmin,zbmax);
  ...
  for(auto& oneObj : sources){
    ...
    if(verbose) cout << "Fit source " << nobj << " with Id " << oneObj->spec << " \r " << flush;
    nobj++;
    // Correct the observed magnitudes and fluxes with the coefficients given in APPLY_SHIFTS
    if(shifts0.size()==(size_t)imagm)oneObj->adapt_mag(shifts0,shifts1);
    // Correct the observed magnitudes and fluxes with the coefficients found by auto-adapt
    ...
    oneObj->fit(fullLib,flux, valid,funz0,bp);
    // Try to remove some bands to improve the chi2, only as long as the chi2 is above a threshold
    oneObj->rm_discrepant(fullLib,flux,valid,funz0,bp,thresholdChi2,verbose);
    // Generate the marginalized PDF (z+physical parameters) from the chi2 stored in each SED
    oneObj->generatePDF(fullLib,valid,fltColRF,fltREF, zfix);
    // Interpolation of Z_BEST and ZQ_BEST (zmin) via Chi2 curves, put z-spec if ZFIX YES  (only gal for the moment)
...
void PhotoZ::write_outputs(vector<onesource*> sources, const time_t& ti1)
{
  ...
  if (outpdz.compare(nonestring) != 0)
    for(const auto& type : pdftype){ pdf_streams[type].close(); }
  stout.close();
