...
void resample(vector<oneElLambda>& lamb_all, vector<oneElLambda>& lamb_interp,const int origine, const double lmin, const double lmax);
...
int main (int argc, char *argv[])
{
  ...
  */
  ...
  // Filter file
  string filters= (key_analysed["FILTER_FILE"]).split<string>("filters.dat",1)[0];
  // Atmospheric extinction curve
  string atmec= (key_analysed["EXT_CURVE"]).split<string>("extinc_etc.dat",1)[0];
  // Galactic extinction curve
  string galec= (key_analysed["GAL_CURVE"]).split<string>("CARDELLI",1)[0];
  // Output file
  string output= (key_analysed["OUTPUT"]).split<string>("filter_extinc.dat",1)[0];
  ...
  */
  ...
  if(!sfiltIn){throw invalid_argument("Can't open file " + fltFile); }
  vector<flt> allFlt;
  ...
  if((galec.compare("SB_calzetti")==0) || (galec.compare("calzetti")==0))rv=4.05;
  ...
  for(int k=0;k<int(allFlt.size());k++){
    ...
    if (atmec.compare("NONE") != 0)aint.push_back(comp_ext(allFlt[k],Atmospheric_Ext));else aint.push_back(99.);
  ...
  */
  ...
  for(int k=0;k<int(allFlt.size());k++){
     string shortName =  (allFlt[k].name).substr(((allFlt[k].name).rfind("/")+1));
     cout << setw(30) << shortName << " "  << setw(12) << aint[k]  << " "  << setw(12)<< albdav[k] << " " << setw(12) << albd[k] << endl;
   }
...
double comp_ext(flt& oneFlt, ext& oneExt){
  ...
  for(size_t i=0; i<new_lamb_flt.size()-1; i++){
    ...
    fint += ( new_lamb_flt[i].val + new_lamb_flt[i+1].val ) /2.  * (new_lamb_flt[i+1].lamb - new_lamb_flt[i].lamb);
    // Integral of the transmission by the filter x extinction
    aint += ( new_lamb_flt[i].val + new_lamb_flt[i+1].val ) * (new_lamb_flt[i+1].lamb-new_lamb_flt[i].lamb) * (new_lamb_ext[i].val+new_lamb_ext[i+1].val) / 4.;
  }

  // clean
  lamb_all.clear(); new_lamb_flt.clear(); new_lamb_ext.clear(); 
...
void resample(vector<oneElLambda>& lamb_all, vector<oneElLambda>& lamb_interp,const int origine, const double lmin, const double lmax) {
    ...
    for (vector<oneElLambda>::iterator it=lamb_interp.begin(); it<lamb_interp.end(); ++it){
      ...
      for (vector<oneElLambda>::iterator kt=it; kt<lamb_interp.end(); ++kt){
        ...
        if(kt->ori == origine){nextEl = *kt;  break;}
