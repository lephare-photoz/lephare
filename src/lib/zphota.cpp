...
int main (int argc, char *argv[])
{
  ...
  clock_t t1,t2;
  t1=clock();
  cout << endl << "Starting times in sec: " <<  (((float)t1) / CLOCKS_PER_SEC) <<endl;
  ...
  */
  ...
  string list_keywords[] =  {"CAT_IN","INP_TYPE","CAT_MAG","CAT_FMT","CAT_LINES","ZPHOTLIB","PARA_OUT","CAT_OUT","VERBOSE","CAT_TYPE","ERR_SCALE","ERR_FACTOR","BD_SCALE","GLB_CONTEXT","FORB_CONTEXT","MASS_SCALE","MAG_ABS","MAG_ABS_AGN","MAG_REF","NZ_PRIOR","ZFIX","Z_INTERP","EXTERNALZ_FILE","RM_DISCREPANT_BD","Z_RANGE","EBV_RANGE","DZ_WIN","MIN_THRES","PROB_INTZ","SPEC_OUT","FIR_LIB","FIR_LMIN","FIR_CONT","FIR_SCALE","FIR_FREESCALE","FIR_SUBSTELLAR","MABS_METHOD","MABS_CONTEXT","MABS_REF","MABS_FILT","MABS_ZBIN","RF_COLORS","M_REF","Z_METHOD","APPLY_SYSSHIFT","AUTO_ADAPT","ADAPT_BAND","ADAPT_LIM","ADAPT_CONTEXT","ADAPT_ZBIN","PDZ_OUT","PDZ_TYPE","PDZ_MABS_FILT","ADD_EMLINES","ADDITIONAL_MAG","LIMITS_ZBIN","LIMITS_MAPP_REF","LIMITS_MAPP_SEL","LIMITS_MAPP_CUT"};
  //Number of keywords
  ...
  */
  ...
  if(autoadapt){
    ...
    cout << " Done " << endl;
    string offsets;
    for(int k=0;k< photoz.imagm; k++) offsets=offsets + to_string(a0[k]) + "," ;
    ...
    for (int k=0;k<photoz.imagm; k++){a0.push_back(0.);a1.push_back(0.);}
  }
  ...
  */
  ...
  cout << endl << "Time to run the code in sec, sum over all threads: " << diff <<endl;
