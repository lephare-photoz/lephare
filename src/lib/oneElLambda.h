/*
  17/11/2014
  Class to store one element with lambda, value, origin (used for create vector in filter and SED)
*/
...
public:
  double lamb, ///< lambda value
    ...
    ori;       ///< index : index referring to filter transmission (ori=0), or SED flux (ori=1), or attenuation law (ori=2), or opacity (ori=3), or emission line (ori=5)
  ...
  inline bool operator<(const oneElLambda& rhs) const {return lamb < rhs.lamb;}
};
