...
double cosmo::distMet(double z) const {
  ...
  if(l0==0){
    ...
    for(int i=0; i<50; i++){
      ...
      double Ez= sqrt(om0*pow((1.+zi),3.)+(1-om0-l0)*pow((1.+zi),2.)+l0);
      sum = sum + dz/Ez;
    }
    dmet =  ckms/(h0*ao) * sum;
      
  }else{
    throw runtime_error( "Cosmology not included : h0=" + to_string(h0)+" Om0="+to_string(om0)+" l0="+to_string(l0));
  }
...
vector<double> zgrid(int gridType, double dz, double zmin, double& zmax) { 
  ...
  if(zmax < zmin){
    throw invalid_argument("You are probably using the old parametrisation of Z_STEP since Z MIN > Z MAX in Z_STEP.");
  }
...
int indexz(const double red, const vector<double>& gridz){
  ...
  //  auto val = binary_search(gridz.begin(), gridz.end(), red);//if gridz guaranteed sorted
