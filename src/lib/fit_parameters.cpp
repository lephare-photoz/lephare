...
int main(int argc, char *argv[]) {
  // Stock the ssfr value in this (even if it was not the original design of the
  // object
  LFdata SSFRone;
  ...
  while (getline(ssfrfiles, lit)) {
    ...
    while (getline(onessfrst, lit)) {
      // put the line into the stream ss again
      stringstream ss(lit);
      ss >> zmin >> zmax >> a >> a >> a >> a >> a >> a >> a >> a >> a >> a >>
          a >> a >> a >> a >> a >> SSFRone.value >> SSFRone.errp >>
          SSFRone.errm;
      SSFRone.z = (zmin + zmax) / 2.;
      SSFRone.M = massmed;
      // cout << " ssfr " <<  SSFRone.z << " " << SSFRone.value  << "  " <<
      // SSFRone.errp << " " <<  SSFRone.errm << endl;
    ...
    if (successmini) {
      ...
      for (int k = 0; k < 3; k++) {
        ...
        if (!min.UserState().Parameter(k).IsFixed()) {
          ...
          cout << "Parameter: " << k << " Value: " << min.UserState().Value(k)
               << " Negative err: " << errminos.first
               << " Positive err: " << errminos.second << endl;
        } else {
          cout << "Parameter: " << k << " Value: " << min.UserState().Value(k)
               << endl;
        }
