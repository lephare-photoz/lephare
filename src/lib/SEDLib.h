...
private:
  string config, ///< configuration file (arg -c at the command line)
    typ; ///< type of the SED instance (arg -t at the command line), either S, G, or Q for a star, galaxy, or QSO template, respectively
  
protected:
  ofstream sdocOut, ///< output stream to store the doc in \a #docFile
    sbinOut, ///< output stream to store the binary SED library in \a #binFile
    sphysOut; ///< output stream to store the binary SED library in \a #physFile (only for typ=GAL)
  string ageFile;///< optional file providing the selection of ages to keep in the SED library;
  double agemin, agemax;
  vector<double> ageSel;
  string physFile; ///< output file name, locate in $LEPHAREWORK/lib_bin/; used only for typ GAL
  
public:
  /*
   * vector, containing the SEDs for the particular type, that will be unique in the derived class
   */
  ...
  /// @param sedFormat format of \a sedFile : can be B(C03), P or F for PEGASE type, or else plain ASCII
...
SEDLib<T>::SEDLib(string conf, string t){
  ...
  if(typ[0]=='S' || typ[0]=='s'){ typ = "STAR"; }
  else if(typ[0]=='Q' || typ[0]=='q'){ typ = "QSO"; }
  else {typ = "GAL";}
...
template <class T>
SEDLib<T>::SEDLib(keymap & key_analysed, string config, string t) : SEDLib(config, t){
  path="/sed/" + typ + "/";
  ...
  if (typ=="GAL"){
	...
	agemin =((key_analysed["AGE_RANGE"]).split_double(to_string(INVALID_PHYS), 2))[0];
	agemax =((key_analysed["AGE_RANGE"]).split_double(to_string(INVALID_PHYS), 2))[1];
  }
...
void SEDLib<T>::write_SED_lib(){
  ...
  for (typename vector<T>::iterator it=allSED.begin(); it<allSED.end(); ++it){
...
void SEDLib<T>::read_model_list(){
  ...
  while(getline(smod, lit)){
    ...
    if(test_first_char(lit)) {
      ...
      //Read the name of the SED and the format which need to be used to read it
      ss >> nameSED ;
...
template <>
void SEDLib<GalSED>::readSED(string sedFile, string sedFormat, int nummod, string type){
  resultSED.clear();
