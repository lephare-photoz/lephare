/*
 *  Class with all the tools used to create the grid of SED in e(b-v), redshift, etc 
 *  Create the library of modeled magnitude and k-corrections
 */
...
// Constructor of the basis class which read the keywords common to the QSO/STARS/GAL
Mag::Mag(keymap & key_analysed){
  ...
  */
  ...
  // extinction laws, multiple laws are possible, number of expected laws unknown in advance -> -1
  extlaw = (key_analysed["EXTINC_LAW"]).split_string("calzetti.dat",-1);
  nextlaw = int(extlaw.size());
  read_ext();
  
  // possible E(B-V) values, multiple values are possible, number of expected values unknown in advance -> -1
...
void Mag::open_files(){
  ...
  if(!ssedIn){throw invalid_argument("Can't open file " + sedlibFile); }
  ...
  // Check if file is opened
  if(!sdocOut){throw invalid_argument("Can't open file " + docFile); }
  ...
  // Check if file is opened
  if(!sbinOut){throw invalid_argument("Can't open file " + binOutFile); }
  ...
  if(outasc){
    ...
    // Check if file is opened
    if(!sdatOut){throw invalid_argument("Can't open file " + datFile); }
    // header of the .dat file
    sdatOut <<   "# model ext_law E(B-V) L_T(IR) redshift dist_modulus age record N_filt magnitude_vector kcorr_vector em_lines_fluxes_vector " << endl ;
  }
...
ifstream Mag::open_opa_files(){
  ...
  // Check if file is opened
  if(!stream){throw invalid_argument("Can't open file " + opaListFile); }
  return stream;
}
...
vector<opa> Mag::read_opa(){
  ...
  for(int i=0;i<81;i++) {
    ...
    opa oneOpa(red,name);
    oneOpa.read();
    // Put as last element a lambda at the Lyman-alpha wavelength with transmission=1
    // Meiksin case : remove the last element which is after the Lya line
...
void Mag::read_flt(const string& inputfile){
  ...
  // Check if file is opened
  if(!sfiltIn){throw invalid_argument("Can't open file " + inputfile); }
...
// Read the long wavelength Bethermin+2012 templates to add the dust emission to the BC03 templates
// Associate a b12 SED to each redshift of the grid in redshift
void Mag::read_B12(){

  /*
  IMPORTANT NOTE
  There is one limitation with current implementation of the code:
  If several templates from B12 are used (not the first one by default), the fit and predicted magnitudes will be correct.
  But the best-fit template in the .spec file will be off in FIR since it is based on the z=0 full template to be reconstructed
  No easy fix yet.
  */
  ...
  if(!b12mod)throw invalid_argument("Can't open Bethermin+12 list "+b12List);

  // Create a list of SED with the B12 templates. Need one SED for each redshift of gridz. 
  ...
  while(getline(b12mod, lit)){
    ...
    if(test_first_char(lit)) {
      ...
      // First step
      if(gr==0){B12SED.push_back(oneSEDFIR); gr++;}

      // We need one SED for each step of the gridz
      while(((gridz[gr]+gridz[gr+1])/2.)<=b12z && gr>0 && gr<gridz.size()-1){
        B12SED.push_back(oneSEDFIR); gr++;}
    }
  }
  // Add the SED to go from the last redshift in B12 template to the last redshift of the grid
...
void Mag::def_zgrid(){
  ...
  // Loop over the redshift grid and measure the age of the Universe and the distance modulus
...
void Mag::write_doc(){
  ...
  sdocOut <<  "AB_COR   " ;
  for (vector<flt>::iterator itf=allFlt.begin(); itf<allFlt.end(); ++itf)sdocOut << itf->abcorr() << "," ;
  sdocOut <<  endl << "FLUX_COR   " ;
  for (vector<flt>::iterator itf=allFlt.begin(); itf<allFlt.end(); ++itf)sdocOut << itf->fcorrec() << "," ;
  sdocOut << endl << "ZGRID_TYPE   " << gridType << endl;
  sdocOut << "Z_STEP   " << dz << "," << zmin << "," << zmax << endl;
  sdocOut << "COSMOLOGY   " << lcdm << endl;
  sdocOut << "EXTINC_LAW   " ;
  for (int k=0 ; k<nextlaw ; k++){sdocOut << extlaw[k] << "," ;};
  sdocOut << endl << "MOD_EXTINC   ";
  for (int k=0 ; k<nextlaw ; k++){sdocOut << modext[k*2] << ","<< modext[2*k+1] << "," ;};
  sdocOut << endl << "EB_V   " ;
...
GalMag::GalMag(keymap & key_analysed) : Mag(key_analysed){
  ...
  if(emlines.substr(0,6).compare("EMP_UV")!=0 && emlines.substr(0,7).compare("EMP_SFR")!=0 && emlines.substr(0,4).compare("PHYS")!=0 && emlines.substr(0,2).compare("NO")!=0 ){
...
void GalMag::read_SED(){
  ...
  // find the end of the stream for the binary input file, then go back at the beginning
  ...
  while(ssedIn.tellg() < length ){
	...
	cout << "Possible to subdivide the library if necessary, or reduce the parameter space." << endl;
	throw runtime_error();
      }
    }
#endif
...
vector<GalSED> GalMag::make_maglib(GalSED& oneSED){
  ...
  for(int i=0; i<nextlaw;i++){
    ...
    for(int j=0; j<nebv;j++){
      // loop over each fraction of emission line flux (add a dispersion in emission lines as a new template)
	...
	for (size_t k=0; k<gridz.size(); k++){
	  ...
	  // Remove all cases with extinction not in the right model range
          // The condition i==0 only means that for null extinction the templates are computed only once,
          // using the first extinction law, and it does not matter which one this extinction law is.
	  if((ebv[j]<1.e-10 && i==0) || (ebv[j]>0 && oneSED.nummod>=modext[i*2] && oneSED.nummod<=modext[i*2+1])){
	    ...
	    if(gridT[k]>oneSEDInt.age){
	      ...
	      // Difference between the integrated flux with and without extinction (without is computed just above)
	      // flux integrate of the lambda range -> erg/s/cm2. It was for the source at 10pc, and then convert erg/s in Lsol 
              double dL = (LbeforeExt - oneSEDInt.trapzd())/Lsol*(4*pi*100*pow(pc,2));
	      if(oneSEDInt.ltir<0 && dL>0)oneSEDInt.ltir=log10(dL);
	      // Rescale the B12 to the right dust luminosity (with energy balance) and sum to the stellar continuum is option on.
	      if(addDust[0]=='Y' || addDust[0]=='y'){oneSEDInt.sumSpectra(B12SED[k],dL);}
		
	      // Opacity applied in rest-frame, depending on the redshift of the source
	      oneSEDInt.applyOpa(opaAll);

	      // redshift the SED, and restrict it to the union of the filters support.
	      oneSEDInt.redshift();
		...
		// Generate intermediate EM SED, since original one must not change
  ...
  for (size_t k=0; k<allSED.size(); k++){
	...
	{allSED[k].kcorr.push_back(allSED[k].mag[itf] - allSED[k].distMod - magko[itf]);}
    }
  }
  return allSED;
}
...
void GalMag::print_info(){
  ...
  cout << "# GAL_LIB_IN    :" << lepharework + "/lib_bin/" + lib + "(.doc & .bin)" << endl;
  cout << "# GAL_LIB_OUT   :" << lepharework + "/lib_mag/" + colib + "(.doc & .bin)"   << endl;
  cout << "# ZGRID_TYPE   :" << gridType << endl;
  cout << "# Z_STEP   :" << dz << " " << zmin << " " << zmax << endl;
  cout << "# COSMOLOGY   :" << lcdm << endl;
  cout << "# EXTINC_LAW   :" ;
  for (int k=0 ; k<nextlaw ; k++){cout << extlaw[k] << " " ;};
  cout << endl << "# MOD_EXTINC   :";
  for (int k=0 ; k<nextlaw ; k++){cout << modext[k*2] << " "<< modext[2*k+1] << " " ;};
  cout << endl << "# EB_V   :" ;
...
// constructor for the QSO adding the missing keywords from the basis constructor
...
void QSOMag::print_info(){
  ...
  cout << "# QSO_LIB_IN    :" << lepharework + "/lib_bin/" + lib + "(.doc & .bin)" << endl;
  cout << "# QSO_LIB_OUT   :" << lepharework + "/lib_mag/" + colib + "(.doc & .bin)"   << endl;
  cout << "# ZGRID_TYPE   :" << gridType << endl;
  cout << "# Z_STEP   :" << dz << " " << zmin << " " << zmax << endl;
  cout << "# COSMOLOGY   :" << lcdm << endl;
  cout << "# EXTINC_LAW   :" ;
...
// Constructor of the stars adding the keywords missing in the basis class constructor
...
void StarMag::read_SED(){
  ...
  while(ssedIn.tellg() < length ){
    ...
    // Info screen
    if(verbose)cout << "Read SED " << oneSED.name << " z " <<  setw(6) << oneSED.red  << "  \r " << flush;
...
vector<StarSED> StarMag::make_maglib(const StarSED& sed){
  ...
  //return singleton vector in order to have the same structure as for QSO and Gal
