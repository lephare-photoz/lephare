...
double schechterFCN::operator()(const vector<double> &par) const {
  ...
  if (par[0] > par[1]) {
    // Calcule d'abord la mediane et le norma dans le cas ou on ajoute un
    // starburst
    double prenorma = 0;
    double premed = -99;
    if (ftype == 4 || ftype == 5) {
      ...
      premed = log10(schechpre.schechIntLD(0., 20.) /
                     schechpre.schechInt(0., 20.));  // schechpre.Mmed(0.,20.) ;
      // prenorma=par[3];
      // premed=par[2];
      /*if(fabs(premed-par[2])>0.3){
        cout << "Prob " << premed-par[2] <<endl;
        cout << " Ben, c'est la med ? " << premed << " Mstar " << par[2]  <<
        endl; cout << " Ben, c'est la norma ? " << prenorma <<  " p1 " << par[3]
        << endl; cout << " Reste " <<  " a1 " << par[0] << " a2 " << par[1] <<
        " p2 " << par[4] << endl;
        }*/
    }
