...
class SchechterFunc {
 ...
 public:
  ...
  SchechterFunc(const int type, const double alpha1, const double alpha2,
                const double mstar, const double phi1, const double phi2)
      : ftype(type),
        falpha1(alpha1),
        falpha2(alpha2),
        fmstar(mstar),
  ...
  // Constructor of the schechter function,
  // arguments -> type(LF/MF/SFRF), a vector of double including the 5 schechter
  // parameters
  ...
  // Constructor of the schechter function,
  // arguments -> type(LF/MF/SFRF), 3 vectors of double, including the 5
  // schechter parameters, the negative and positive errors associated
  SchechterFunc(const int type, const vector<double> para,
                const vector<double> errn, const vector<double> errp)
      : ftype(type),
        falpha1(para[0]),
        falpha2(para[1]),
        fmstar(para[2]),
  ...
  // Constructor of the schechter function,
  // arguments -> type(LF/MF/SFRF), vector of double[3], including the 5
  // schechter parameters, the negative and positive errors associated
  ...
  double chtype(const int intype) {
    ftype = intype;
    return 0;
  }
