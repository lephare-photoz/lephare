...
void get_lephare_env() {
   ...
   if(temp != NULL){lepharedir = string(temp);}else{cout << "Environment variable LEPHAREDIR not defined, need to stop.";}   
   temp = getenv(LEPHAREWORK.c_str());
   if(temp != NULL){lepharework = string(temp);}else{cout << "Environment variable LEPHAREWORK not defined, use $LEPHAREDIR/work." << endl; lepharework = lepharedir + "/work";}
...
int bdincl(int n,long cont,int max)
{
  ...
  if(sum > 0){
   ...
   for(int i=max-1;i>=n;i--){
    ...
    if ( summ >=0 && i>n){ // The filter is included into the context (summ>0) and the considered filter is not reached
       sum = summ;
