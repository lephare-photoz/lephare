...
int main (int argc, char *argv[])
{
  ...
  */
  ...
  string list_keywords[] =  {"FILTER_REP","FILTER_LIST","TRANS_TYPE","FILTER_CALIB","FILTER_FILE"};
  //Number of keywords
  ...
  string fltRep=((key_analysed["FILTER_REP"]).split_string(lepharedir + "/filt/",1))[0];
  // filter list with the name of the filter file to be oppened
  vector<string> fltFiles   = (key_analysed["FILTER_LIST"]).split_string("flt.pb",-99);
  //Use the number of filters as the reference number of keyword (the following list of values should match 1 or this number)
  ...
  vector<int> calibtyp   = (key_analysed["FILTER_CALIB"]).split<int>("0",ref_size);
  // Output file
  string outputName = (key_analysed["FILTER_FILE"]).split<string>("filters",1)[0];
  ...
  */
  ...
  cout <<  "# FILTER_LIST: ";
  for (int k=0 ; k<ref_size ; k++){cout << fltFiles[k] << " " ;}
  cout <<  endl << "# TRANS_TYPE: ";
  for (int k=0 ; k<ref_size ; k++){cout << transtyp[k] << " " ;}
  cout <<  endl << "# FILTER_CALIB: ";
  for (int k=0 ; k<ref_size ; k++){cout << calibtyp[k] << " " ;}
