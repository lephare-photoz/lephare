...
class flt
{
 ...
 public:
  ...
  ///
  /// The Vega spectrum \f$V(\lambda)\f$ is included in LePhare ($LEPHAREDIR/vega/VegaLCB.sed).
  /// Then \f$\lambda_{eff} = \frac{\int V(\lambda)\, T(\lambda)\, \lambda\, d\lambda}{\int V(\lambda)\, T(\lambda)\, d\lambda}\f$.
  double lambdaEff();
  /// \brief effective wavelength based on a specific calibration SED.
  ///
  /// If \f$C(\lambda)\f$
  ///is a specific calibration SED defined by keyword FILTER_CALIB (see SED#generateCalib), then \f$\lambda_{eff2} = \frac{\int C(\lambda)\, T(\lambda)\, \lambda\, d\lambda}{\int C(\lambda)\, T(\lambda)\, d\lambda}\f$.
  double lambdaEff2();
  /// absolute magnitude of the Sun
  double magsun();
  /// Vega magnitude in this filter: \f$mag(Vega) = 2.5\cdot\log10\left(\frac{\int Vega(\lambda)\, T(\lambda)\, d\lambda}{\int T(\lambda)\, d\lambda}\right)\f$ where \f$Vega(\lambda)\f$ is the Vega SED.
  double vega();

  /*! \brief compute the flux correction based on keyword FILTER_CALIB
   *
   * \f$fcorrec = B_0\cdot\lambda_{eff2}^2\cdot\frac{\int T(\lambda)\,\lambda^{-2}\, d\lambda}{\int C(\lambda)\, T(\lambda)\, d\lambda}\f$,
   * where \f$C(\lambda)\f$ is a calibration function defined through the value of FILTER_CALIB (see SED::generateCalib), \f$\lambda_{eff2}\f$ is the alternative effective wavelength using \f$C(\lambda)\f$ (see flt::lambdaEff2), and
   ...
   *
   * Pour les cas 4 et 5, \f$\lambda_{eff2}\f$ est évaluée en utilisant le calibrator \f$C(\lambda)\f$ sélectionné par FILTER_CALIB=1.
   */
  double fcorrec();

  /// AB-Vega correction \f$ =  -2.5\,\log10\left(\frac{\int F(Vega)\,T(\lambda)\,d\lambda}{\int T(\lambda)\, c\lambda^{-2}\,d\lambda}\right)\f$
  double abcorr();
