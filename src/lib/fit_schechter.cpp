...
vector<keyword> read_command(int argc, char *argv[], int &nb_keywords,
                             string &configfile);
...
vector<vector<double>> minimizationSchech(const vector<LFdata> LFall,
                                          const int k,
...
int main(int argc, char *argv[]) {
  ...
  string list_keywords[] = {"FIT_INPUT",      "FIT_OUTPUT",    "FIT_FIX_ALPHA1",
                            "FIT_FIX_ALPHA2", "FIT_FIX_MSTAR", "FIT_FIX_PHI1",
                            "FIT_FIX_PHI2",   "EDDINGTON",     "FIT_TYPE",
                            "FIX_NORMA",      "FIX_NORMAERR"};
  // Number of keywords
  ...
  vector<double> a1fix, a2fix, phi1fix, phi2fix, mstarfix, zbin, zbmin, zbmax,
      normafix, normafixerr, eddi;
  vector<int> NbGal;
  ...
  */
  ...
  if (temp != NULL) {
    ...
    cout
        << "Environment variable LEPHAREWORK not defined, used local directory."
        << endl;
  }
  ...
  */
  ...
  cout << "Number of keywords read in the command line: " << nb_keywords
       << endl;

  // stop if no config file in the command line
  if (configfile.length() == 0) {
    cout << "No configuration file indicated with the -c option. Need to stop"
         << endl;
    exit(2);
  } else {
  ...
  for (vector<keyword>::iterator it = key_all.begin(); it < key_all.end();
       ++it) {
    // Name of the keyword which match
    int iarg = find_keyword(*it, list_keywords, nb_ref_key);

    // FIT_INPUT:0  FIT_OUTPUT:1 FIT_FIX_ALPHA1:2 FIT_FIX_ALPHA2=3
    // FIT_FIX_MSTAR=4 FIT_FIX_PHI1=5 FIT_FIX_PHI2=6 EDDINGTON=7 FIT_TYPE 8
    // FIX_NORMA 9 FIX_NORMAERR 10
    switch (iarg) {
      case -1:
  ...
  */
  ...
  // Loop over all the keywords
  for (vector<keyword>::iterator it = key_info.begin(); it < key_info.end();
       ++it) {
    // Name of the keyword which match
    ...
    switch (iarg) {
      ...
      case 1:
        NbGal.push_back(
            atoi(((*it).value)
                     .c_str()));  // Number of galaxies used to computed the LF
        break;
      case 2:
        inputNoPara.push_back((*it).value);  // File in which the LF data are
                                             // stored for this redshift bin
        break;
    }
  }
  ...
  */
  ...
  for (int k = 0; k < (int)(phi1fixe.size()); k++) {
    ...
    if (phi2fixe[k] == 0 && fittype != 3 && fittype != 5)
      a2fixe[k] = -5.;  // dont fix alpha 2 if double-epo
    // If alpha2 fixed for a double expo, change the value to be consistent with
    // our definion
    if (a2fixe[k] > -90 && a2fixe[k] < 90 && (fittype == 3 || fittype == 5))
      a2fixe[k] = a2fixe[k] - 3;
  ...
  if ((ref_size != zbmax.size()) || (ref_size != inputNoPara.size()) ||
      (ref_size != NbGal.size())) {
    cout
        << "All the  vectors read in the .info file do NOT have the same size ";
    cout << "Sizes: " << zbmin.size() << " " << zbmax.size() << " "
         << inputNoPara.size() << " " << NbGal.size() << endl;
  }
  ...
  */
  ...
  for (int k = 0; k < (int)zbmin.size(); k++) {
    cout << "# Redshift bin: " << zbmin[k] << " " << zbmax[k]
         << " File: " << inputNoPara[k] << " Nb.Gal: " << NbGal[k] << endl;
  }
  cout << "#######################################" << endl;
  ...
  */
  ...
  for (int k = 0; k < (int)zbmin.size(); k++) {
    ...
    read_LFdata(LFst, LFall, ((zbmin[k] + zbmax[k]) / 2.), k, normafixe[k],
                normafixeerr[k]);
    LFst.close();
  }
  ...
  */
  ...
  for (int k = 0; k < (int)zbmin.size(); k++) {
    ...
    if (fittype >= 2 && fittype <= 5)
      upar.SetLowerLimit(
          0, 0.1);  // only positive sigma for double-expo et gaussienne
    if (fittype == 2 || fittype == 4)
      upar.SetUpperLimit(0, 2);  // upper limit for sigma
    ...
    if (k < 10) {
      out = outputname + char(k + 48) + ".dat";
    } else if (k >= 10 && k < 20) {
      out = outputname + "1" + char(k + 38) + ".dat";
    } else {
      cout << "Too many redshift bins (>20) for a good naming " << endl;
    }
    ...
    if (allPara.size() > 0) {
      ...
      // #pragma omp ordered
      {
        ...
        write_output_fit(outlatex, outpara, out, fittype, allPara, zbmin[k],
                         zbmax[k], NbGal[k], eddi[k]);
      }

    } else {
      cout << "No best fit parameters for the redshift bin:" << zbmin[k] << " "
           << zbmax[k] << endl;
      ofstream outst;
      outst.open(out.c_str());
      outst << "# No best fit parameters for the redshift bin " << endl
            << " -999 -999 " << endl;
      outst.close();
    }
