...
vector<keyword> analyse_keywords(int argc, char *argv[], const string * list_keywords, int nb_ref_key);
void write_output_filter  (ofstream & stfiltfile, ofstream & stfiltdoc, vector<flt> vecFlt);

int SED::nrec=0;     // Even if SED used in the class FLT, still need to instantiate this variable (used in sedtolib)
...
int main (int argc, char *argv[])
{
  ...
  */
  ...
  vector<keyword> key_analysed=analyse_keywords(argc, argv,list_keywords,nb_ref_key);
...
// compute atmospheric extinction
double comp_atm(flt *oneFlt, ext *oneExt){
...
double comp_ext(flt *oneFlt, ext *oneExt){
  ...
  for(size_t i=0; i<new_lamb_flt.size()-1; i++){
    fint += ( new_lamb_flt[i].lamb + new_lamb_flt[i+1].lamb ) /2.  * (new_lamb_ext[i+1].val - new_lamb_ext[i].val);
    aint += ( new_lamb_flt[i].lamb + new_lamb_flt[i+1].lamb ) * (new_lamb_flt[i+1].val-new_lamb_flt[i].val) * (new_lamb_flt[i].val+new_lamb_flt[i+1].val) / 4.;
  }
