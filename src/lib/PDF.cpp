...
void PDF::normalization() {
    ...
    // Renormalize the PDF
    if(somme>0){
      for(size_t k=0;k<vsize;k++){vPDF[k]=vPDF[k]/somme; }
    }
...
double PDF::int_parab() {
  ...
  if(ib==0 || (size_t)ib==(xaxis.size()-1)){
    ...
    // if one of the chi2 necessary for the interpolation k-1 -> k+1 too high, can't interpolate
    if(chi2[ib-1]>1.e9 || chi2[ib]>1.e9 || chi2[ib+1]>1.e9){
     // keep the original value
      xmin=xaxis[ib];
    }else{
      // compute the best value from the parabolic interpolation of chi2   
      // define the step for the interpolation (before and after the considered point)
      dxb=xaxis[ib]-xaxis[ib-1];
      dxa=xaxis[ib+1]-xaxis[ib];
      num =  (chi2[ib-1] - chi2[ib])*pow(dxa,2.) - (chi2[ib+1] - chi2[ib])*pow(dxb,2.);
	...
	// xmin=xaxis[ib+1]-dxb*( (chi2[ib+1]-chi2[ib]) / (chi2[ib+1]-2*chi2[ib]+chi2[ib-1]) +0.5);
        // New generic version with flexible step
...
pair<double, double> PDF::uncMin(double dchi){
  ...
  chiLim=chi2[ib]+dchi;
  // cout << " Start " << ib << "  " << chi2[ib] << "   " << xaxis[ib] << "   "  << chiLim << "   " << endl;

  // Loop over each point of the PDF to find the minimum error 
  for(int k=0; k<ib; k++){
    // When the considered chi2 is below the chi2 limit and the following one above
  ...
  for(size_t k=ib; k<xaxis.size(); k++){
    // When the considered chi2 is above the chi2 limit and the following one below 
    if(chi2[k]<=chiLim && chi2[k+1]>chiLim){
      // Linear interpolation
      double slope = (chiLim-chi2[k])/(chi2[k+1]-chi2[k]);
      // store the minimum error
...
/*
 ...
 First argument is the confidence level to consider, and the second argument is the considered value
 */
pair<double, double> PDF::credible_interval(float level,double val){

  pair<double, double> result;
  vector<double>::iterator bound_val,bound_left_id, bound_right_id,boundpos,boundneg;
  ...
  if(maxid>0){if((xaxis[maxid]-val)>(val-xaxis[maxid-1]))maxid=maxid-1;}
  ...
  // Normalization
    ...
    // If integral of the PDF above the mode is higher than xval/2 and lower than 0.5, enough area on each side
    if(cumul_right>=level/2. && cumul_left>=level/2.){
      upperLevel= cumul_left+level/2.;
      lowerLevel= cumul_left-level/2.;
    // Case with the intergral between the min and the mode which is lower than level/2
    ...
    if(xaxis[indR-1]>val){
        ...
        result.second = linear_interp(upperLevel,cumulant[indR-1], xaxis[indR-1] , cumulant[indR], xaxis[indR]);
    }else{
    ...
    if(xaxis[indL+1]<val){
        result.first = linear_interp(lowerLevel, cumulant[indL] ,xaxis[indL],cumulant[indL+1], xaxis[indL+1]);
    }else{
...
double PDF::levelCumu2x(float level){
  ...
  // Normalization
    ...
    if(indR>0){
        // args in reverse order y, y1, x1, y2, x2, in order to get x instead of y
      result = linear_interp(level,cumulant[indR-1], xaxis[indR-1] , cumulant[indR], xaxis[indR]);
    }else{
        result = xaxis[0];
    }
...
void PDF::secondMax(const double win){
  ...
  // At redshift 0
  if(vPDF[0]>vPDF[1]){
    idx.push_back(0); poss.push_back(1);
  ...
  for (size_t i = 1; i < vsize-1; ++i){
   // Check if it could be a local maximum, the two adjacent bin should be lower
  ...
  // At the maximum redshift
   if(vPDF[vsize-1]>vPDF[vsize-2]){
    idx.push_back(j); poss.push_back(1);
  ...
  sort (idx.begin(),idx.end(),[&](int i,int j) { return possP[i]>possP[j]; });  

  // Loop over the potential maximums
  secondX.clear(); secondP.clear(); secondInd.clear(); 
  for(size_t k=0;k<idx.size();k++){
    ...
    int already=0;
    for(int j=k-1;j>=0;j--){
      if(abs(possX[idx[j]]-possX[idx[k]])<win && poss[idx[j]]==1){already=1; poss[idx[k]]=0;}
    }
    if(already==0){
   ...
   possX.clear(); possP.clear(); possInd.clear(); 
