...
static vector<string> phys_par_names = {"AGE","LDUST","LIR","MASS","SFR","SSFR","COL1","COL2","MREF"};
static vector<string> photoz_par_names = {"MIN_ZG", "MIN_ZQ", "BAY_ZG", "BAY_ZQ"};
// How to associate a name to the corresponding interger
static unordered_map<string,int> maptype = { {"MASS",0},{"SFR",1},{"SSFR",2},{"LDUST",3},{"LIR",4},{"AGE",5},{"COL1",6},{"COL2",7},{"MREF",8},{"MIN_ZG",9},{"MIN_ZQ",10},{"BAY_ZG",11},{"BAY_ZQ",12}};
...
public:
  ...
  unordered_map<string, double> results = {
    {"MASS_BEST", INVALID_PHYS},
    {"SFR_BEST",  INVALID_PHYS},
    {"SSFR_BEST", INVALID_PHYS},
    {"LDUST_BEST",INVALID_PHYS},
    {"LUM_TIR_BEST",  INVALID_PHYS},
    {"AGE_BEST",  INVALID_PHYS},
    {"EBV_BEST",  INVALID_PHYS},
    {"EXTLAW_BEST", INVALID_PHYS},
    {"LUM_NUV_BEST",  INVALID_PHYS},
    {"LUM_R_BEST", INVALID_PHYS},
    {"LUM_K_BEST", INVALID_PHYS},
  };
  unordered_map<string, double> results_emission_lines;
  
  long cont,new_cont;
  vector<double> ab, sab, mab, msab, magm,magm0,absmagPred,magPred,kap, mabs, emabs, ab_ori,mab_ori,abIR, sabIR;
  ...
  array<double, 4> priorLib; // Prior with the range in abs mag gal, abs mag AGN
  ...
  vector<double> massmed,SFRmed,sSFRmed,agemed,Ldustmed,LIRmed,col1med,col2med,ebvmed,Mrefmed;
  ...
  onesource(const int posC, const vector<double>& gridz):onesource()
  {
    ...
    pdfmap[0]     = PDF(3.,   13, 201); // log Stellar Mass  [Mo] : 3 to 13 by 0.05 step
    pdfmap[1]     = PDF(-10., 5,  151); // log SFR [Mo/yr] : -10 to 5 by 0.1 step
    pdfmap[2]     = PDF(-25., -5,  201);// log SFR/Mass [SFR/Mo] : -25 to -5 by 0.1 step
    pdfmap[3]     = PDF(6.,   14, 161); // Ldust [L0]  ! from mag_gal (EB-V, Extlaw)  6 to 14 by 0.05 step
    pdfmap[4]     = PDF(6.,   14, 161); // Llir [L0]   ! from  sedtolib (*.phys)  6 to 14 by 0.05 step
  ...
  vector<size_t> validLib(vector<SED*> &fulllib, const bool &zfix,  const double &consideredZ);
  ...
  void fitIR(vector<SED*> &fulllib,const vector<vector<double>> &flux,const int imagm,  const string fit_frsc, cosmo lcdm);
  double nzprior(const double luv, const double lnir,const double reds, const array<int, 2> bp);
  void rm_discrepant(vector<SED*> &fulllib, const vector<vector<double>> &flux, const vector<size_t> &valid, const double funz0, const array<int, 2> bp, double thresholdChi2, bool  verbose);
  void write_out(vector<SED*> &fulllib,vector<SED*> &fulllibIR, ofstream & stout, vector<string> outkeywords);
  void write_pdz_header(vector<string> pdztype,
			unordered_map<string, ofstream>&  stpdz, const time_t& ti1);
  ...
  best_spec_vec(short sol, vector<SED*> &fulllib,
